package com.fcpc.chibogservices.core;

public class Globals {

    public static final String SERVER_IP = "http://192.168.137.1/";
    public static final String ROOT_URL = SERVER_IP + "fcpc_chibog/";
    public static final String LOGIN_URL = ROOT_URL + "useracct/login.php";
    public static final String BALANCE_URL = ROOT_URL + "useracct/get_current_bal.php";
    public static final String FOOD_URL = ROOT_URL + "products/product_list.php";
    public static final String IMAGE_URL = ROOT_URL + "imageres/";
}
